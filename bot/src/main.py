...
def sign_up(update: Update, context: CallbackContext):
    ...
    """
    LOGIN_MESSAGE = {
        'en': "🙋‍♂️important information 1) Verify your email address to Wenet after sign up (check your spam folder also🙏)  2) Fill in the extra details to your WENET profile as prompted in the same email. When the sign up process is complete the next step is to press /login and start using the app! 😀",
        'gr': "🙋‍Σημαντικές οδηγίες! 1) Επιβεβαίωσε το email σου αφου έχεις κάνει εγραφή (κοίταξε και στα ανεπιθύμητα μηνύματα🙏) 2) Συμπλήρωσε τις επιπλέον πληροφορίες στο προφίλ του Wenet καθοδηγούμενος απο το ίδιο email. 'Οταν ολοκληρωθεί η διαδικασία πάτησε /login για να χρησιμοποιήσετε την εφαρμογή! 😀 ",
        'tr': "önemli bilgiler 1) Kaydolduktan sonra e-posta adresinizi Wenet e doğrulamayı unutmayın (spam klasörünüzü de kontrol edin🙏) 2) Aynı e-postada istendiği gibi bir sonraki adım için gerekli olan ekstra ayrıntıları WENET profilinize girin! Kayıt işlemi tamamlandığında, bir sonraki adım /login e basmak ve uygulamayı kullanmaya başlamaktır!😀"
    }
